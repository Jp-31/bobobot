...
import telegram
import time
import natalie_bot.modules.sql.mention_sql as sql
...
from natalie_bot.modules.helper_funcs.chat_status import user_admin, can_message
...
def ping_list(update: Update, context: CallbackContext):
    ...
    if last_execution != False:
        ...
        for i in range(len(all_handlers)):
            if i != 0 and (i % 10) == 0:
                ...
                for keyword in all_handlers[(i - 10):i]:
                    try:
                        user = context.bot.get_chat_member(keyword.chat_id, keyword.user_id)
                    except:
                        user.user.id = 000000000
        ...
        if not ping_list == PING_STRING:
            update.effective_message.reply_text(ping_list.format(mention_html(msg_user.id, msg_user.first_name), 
                                                                 chat_name), parse_mode=telegram.ParseMode.HTML)
    else:
        update.effective_message.reply_text("Command has been used in the last 10 minutes.\n\nIn terms of "
                                            "Anti-Spam measurements there's a 10 minute cooldown on this command.")
        return
...
@run_async
def pingme(update: Update, context: CallbackContext):
    message = update.effective_message  # type: Optional[Message]
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
    ...
    if subscribe_user == True:
        ...
        message.reply_text(subs, parse_mode=ParseMode.HTML)
...
@run_async
def unpingme(update: Update, context: CallbackContext):
    message = update.effective_message  # type: Optional[Message]
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
...
@run_async
@user_admin
def unping_all(update: Update, context: CallbackContext):
    message = update.effective_message  # type: Optional[Message]
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
